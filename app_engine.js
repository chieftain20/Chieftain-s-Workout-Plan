...
function calculateAnthropometrics(m) {
  ...
  if (height > 0) {
    ...
    if (leanMass !== null) {
      ...
      // Hossein trains 5 sessions a week (4 heavy lifting + 1 core stability)
      // Realistic annual gains: ~3.8 kg/yr for intermediate/advanced with high adherence
      yearsToPotential = Number((remainingMusclePotential / 3.8).toFixed(1));
    }
  }
  ...
  if (leanMass !== null) {
    ...
    // 5 structured workouts/week = activity multiplier ~ 1.55 (Moderately Active)
    tdee = Math.round(bmr * 1.55);
    // Body recomposition & fat loss with muscle preservation: 450-500 kcal deficit
    targetCalories = tdee - 500;
    // 2.2g protein per kg of Lean Mass (optimal for hypertrophy & fat loss)
    targetProtein = Math.round(leanMass * 2.2);
  }
...
function getAnthropometricReferenceRanges(height, gender) {
  ...
  return {
    bodyFat: {
      ...
      goldenRange: isFemale ? '15% – 20%' : '8% – 12%',
      unit: '%',
      evalStatus: (val) => {
        if (!val) return null;
        if (isFemale) {
          if (val <= 20) return { label: '👑 طلایی / فیتنس', cls: 'lab-status-golden' };
    ...
    neck: {
      healthRange: isFemale ? '32.0 – 36.0 cm' : '37.0 – 41.0 cm',
      goldenRange: isFemale ? '33.0 – 35.0 cm' : `${(H * 0.21).toFixed(1)} – ${(H * 0.23).toFixed(1)} cm (تثلیث استیو ریوز)`,
      unit: 'cm',
      evalStatus: (val) => {
        if (!val) return null;
        if (!isFemale) {
          ...
          if (val >= 32 && val <= 36) return { label: '🟢 نرمال سلامت', cls: 'lab-status-healthy' };
          return { label: '⚠️ بررسی بیشتر', cls: 'lab-status-attention' };
        }
      }
    },
    loveHandle: {
      healthRange: '< 1.10',
      goldenRange: '1.00 – 1.05 (بدون چربی پهلو 👑)',
      unit: '',
      evalStatus: (val) => {
        if (!val) return null;
        if (val <= 1.05) return { label: '👑 پهلوی تراشیده V-Cut', cls: 'lab-status-golden' };
    ...
    swr: {
      healthRange: '1.30 – 1.45',
      goldenRange: '1.55 – 1.65 (هدف: ۱.۶۱۸)',
      unit: '',
      evalStatus: (val) => {
        if (!val) return null;
    ...
    chest: {
      healthRange: `${Math.round(H * 0.50)} – ${Math.round(H * 0.55)} cm`,
      goldenRange: `${Math.round(H * 0.58)} – ${Math.round(H * 0.62)} cm`,
      unit: 'cm',
      evalStatus: (val) => {
        if (!val) return null;
        const target = H * 0.58;
    ...
    armFlexed: {
      ...
      goldenRange: `${(H * 0.22).toFixed(1)} – ${(H * 0.24).toFixed(1)} cm`,
      unit: 'cm',
      evalStatus: (val) => {
        if (!val) return null;
        const target = H * 0.22;
    ...
    armRelaxed: {
      healthRange: `${(H * 0.16).toFixed(1)} – ${(H * 0.18).toFixed(1)} cm`,
      goldenRange: `${(H * 0.20).toFixed(1)} – ${(H * 0.22).toFixed(1)} cm`,
      unit: 'cm',
      evalStatus: (val) => {
        if (!val) return null;
    ...
    waist: {
      ...
      goldenRange: `${(H * 0.42).toFixed(1)} – ${(H * 0.45).toFixed(1)} cm`,
      unit: 'cm',
      evalStatus: (val) => {
        if (!val) return null;
        if (val <= H * 0.45) return { label: '👑 کمر باریک و کات', cls: 'lab-status-golden' };
    ...
    whr: {
      ...
      goldenRange: isFemale ? '0.70 – 0.75' : '0.80 – 0.84',
      unit: '',
      evalStatus: (val) => {
        if (!val) return null;
        if (isFemale) {
          if (val <= 0.75) return { label: '👑 ساعت‌شنی ایده‌آل', cls: 'lab-status-golden' };
          if (val <= 0.80) return { label: '🟢 استاندارد سلامت', cls: 'lab-status-healthy' };
          return { label: '🔴 تجمع چربی', cls: 'lab-status-attention' };
        } else {
    ...
    thigh: {
      ...
      goldenRange: `${(H * 0.32).toFixed(1)} – ${(H * 0.35).toFixed(1)} cm`,
      unit: 'cm',
      evalStatus: (val) => {
        if (!val) return null;
    ...
    calves: {
      healthRange: `${(H * 0.18).toFixed(1)} – ${(H * 0.20).toFixed(1)} cm`,
      goldenRange: `${(H * 0.21).toFixed(1)} – ${(H * 0.23).toFixed(1)} cm`,
      unit: 'cm',
      evalStatus: (val) => {
        if (!val) return null;
...
function renderBodyMetricsView() {
  ...
  if (list.length === 0) {
      ...
      <div style="background:#111a2e; border:1px solid rgba(99,102,241,0.35); border-radius:var(--radius-lg); padding:16px; margin-bottom:16px; box-shadow:0 4px 20px rgba(0,0,0,0.35);">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; border-bottom:1px solid rgba(255,255,255,0.08); padding-bottom:8px; flex-wrap:wrap; gap:8px;">
          <div style="display:flex; align-items:center; gap:8px;">
            ...
            <div>
              ...
              <div style="font-size:11px; color:#94a3b8; margin-top:2px;">سقف ژنتیکی بدون دارو (مدل دکتر کیسی بات) کالیبره شده با برنامه تمرینی شما (۵ روز در هفته)</div>
            </div>
          </div>
        ...
        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(220px, 1fr)); gap:10px; margin-bottom:10px;">
          ...
          <div style="background:rgba(15,23,42,0.6); border:1px solid rgba(255,255,255,0.06); border-radius:10px; padding:12px;">
            ...
            <div style="font-size:11px; color:#94a3b8; line-height:1.5;">
              محاسبه شده بر اساس ساختار برنامه شما: <b>۴ روز باشگاه + ۱ روز خانه (۵ روز در هفته)</b> با تغذیه و ریکاوری هدفمند.
            </div>
          </div>
        </div>
    ...
    // --- Special Diagnostic Box: Love Handles, Somatotype & Steve Reeves Golden Trinity ---
    ...
    const loveHandleBoxHtml = `
      <div style="background:linear-gradient(145deg, #0e172a, #162035); border:1px solid ${isLoveHandleProminent ? 'rgba(244,63,94,0.35)' : 'rgba(56,189,248,0.35)'}; border-radius:var(--radius-lg); padding:16px; margin-bottom:16px; box-shadow:0 4px 20px rgba(0,0,0,0.35);">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; border-bottom:1px solid rgba(255,255,255,0.08); padding-bottom:8px; flex-wrap:wrap; gap:8px;">
          <div style="display:flex; align-items:center; gap:8px;">
            <span style="font-size:22px;">🎯</span>
            <div>
              <h3 style="font-size:14.5px; font-weight:900; color:${isLoveHandleProminent ? '#fb7185' : '#38bdf8'}; margin:0;">
                آنالیز توزیع چربی موضعی (لاو هندل) و تقارن کلاسیک استیو ریوز
              </h3>
              <div style="font-size:11px; color:#94a3b8; margin-top:2px;">
                تشخیص تیپ بدنی ساختاری (${prof.name}) و پروتکل هدفمند رفع بالشتک‌های پهلو
              </div>
            </div>
          </div>
          <span style="font-size:11px; background:${isLoveHandleProminent ? 'rgba(244,63,94,0.15)' : 'rgba(56,189,248,0.15)'}; color:${isLoveHandleProminent ? '#f43f5e' : '#38bdf8'}; border:1px solid ${isLoveHandleProminent ? 'rgba(244,63,94,0.3)' : 'rgba(56,189,248,0.3)'}; padding:3px 10px; border-radius:8px; font-weight:800;">
            ${isLoveHandleProminent ? '⚠️ لاو هندل شناسایی شد' : '🟢 توزیع نرمال'}
          </span>
        </div>

        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(240px, 1fr)); gap:12px; margin-bottom:12px;">
          <!-- Card 1: Love Handles Diagnosis -->
          <div style="background:rgba(15,23,42,0.6); border:1px solid rgba(255,255,255,0.06); border-radius:10px; padding:12px;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
              <span style="font-size:12px; font-weight:800; color:#fb7185;">🍩 شاخص چربی پهلو و زیر شکم (Love Handle):</span>
            ...
            <div style="font-size:11.5px; color:#cbd5e1; line-height:1.6;">
              ${isLoveHandleProminent ? 
                `دور پهلو و زیر شکم شما (<b>${curSci.flankCirc || 101}cm</b>) حدود <b>${curSci.loveHandleDelta || 13}cm</b> از دور کمر باریک (<b>${current.waist || 88}cm</b>) عریض‌تر است. این اختلاف نشان‌دهنده تجمع چربی موضعی سرسخت در بالای استخوان لگن (Love Handles) است.` :
                `توزیع چربی در میان‌تنه یکنواخت است و چربی موضعی قابل‌توجهی در پهلوها دیده نمی‌شود.`}
            </div>
            <div style="margin-top:8px; background:rgba(0,0,0,0.25); border-radius:6px; padding:6px 10px; font-size:10.5px; color:#94a3b8;">
              💡 <b>تیپ بدنی:</b> مزومورف - اندومورف عضلانی با گیرنده‌های آلفا-۲ آدرنرژیک در بافت پهلو که آخرین چربی‌های آب‌شونده در کات هستند.
            </div>
          </div>

          <!-- Card 2: Somatotype & Steve Reeves Golden Trinity -->
          <div style="background:rgba(15,23,42,0.6); border:1px solid rgba(255,255,255,0.06); border-radius:10px; padding:12px;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
              <span style="font-size:12px; font-weight:800; color:#facc15;">👑 تثلیث طلایی استیو ریوز (Neck / Arm / Calf):</span>
              <span style="font-size:11px; color:#38bdf8;">هدف: برابری سه عضو</span>
            </div>
        ...
        <div style="background:rgba(15,23,42,0.8); border:1px solid rgba(245,158,11,0.25); border-radius:10px; padding:10px 14px; font-size:11px; color:#cbd5e1; line-height:1.6;">
          🔥 <b>پروتکل مربیگری اختصاصی برای محو کردن لاو هندل (چربی پهلو):</b>
