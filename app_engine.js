...
function unlockAdminMode(pwd) {
  const validPasswords = ['gym', 'chieftain20', 'admin123'];
  if (validPasswords.includes(pwd)) {
    localStorage.setItem('chieftain_admin_unlocked', 'true');
    updateAdminUI();
    showToast('👑 حالت مدیریت ادمین فعال شد!');
    return true;
  }
  return false;
}
...
function lockAdminMode() {
  ...
  updateAdminUI();
  showToast('🔒 از حالت مدیریت ادمین خارج شدید.');
}
...
function getEffectiveCloudKey() {
  ...
  if (typeof currentAuthUser !== 'undefined' && currentAuthUser) {
    const emailLower = (currentAuthUser.email || '').toLowerCase();
    if (emailLower.includes('hossein') || emailLower.includes('chieftain')) return 'hossein';
    if (emailLower.includes('morvarid')) return 'morvarid';
    return 'user_' + (currentAuthUser.id ? currentAuthUser.id.substring(0, 8) : 'sync');
  }
  // If user has unlocked admin mode
  if (typeof isAdminUnlocked === 'function' && isAdminUnlocked()) {
    if (activeProfileId === 'hossein_chieftain') return 'hossein';
    if (activeProfileId === 'morvarid') return 'morvarid';
  }
  // Security guard for guests/strangers: never default to hossein or morvarid
  if (savedKey === 'hossein' || savedKey === 'morvarid') {
    if (typeof isAdminUnlocked !== 'function' || !isAdminUnlocked()) {
      savedKey = 'guest_' + Math.random().toString(36).substring(2, 8);
...
async function quickCloudSyncAction(btn) {
  ...
  showToast('☁️ در حال ارسال تمام داده‌ها و لاگ‌ها به سرور ابری...');
  try {
    const pushed = await pushToCloudStorage(true);
    await pullFromCloudStorage(true);
    if (pushed) {
      showToast('✅ تمام اطلاعات، لاگ‌ها و ست‌ها در سرور ابری ذخیره شدند! 🟢');
    } else {
      showToast('✅ اطلاعات در حافظه دستگاه ذخیره شد و آماده همگام‌سازی ابری است.');
    }
  } catch(e) {
    showToast('⚠️ ذخیره محلی با موفقیت انجام شد (اتصال اینترنت را بررسی کنید).');
  } finally {
    if (btn) {
      btn.style.opacity = '1';
...
function updateAuthUI(user) {
  ...
  if (user) {
    ...
    if (emailLower.includes('hossein') || emailLower.includes('chieftain')) {
      localStorage.setItem('chieftain_admin_unlocked', 'true');
      if (appTitleEl) appTitleEl.innerText = 'برنامه تمرینی تخصصی حسین';
    } else if (emailLower.includes('morvarid')) {
      if (appTitleEl) appTitleEl.innerText = 'برنامه تمرینی تخصصی مروارید';
    } else {
      if (appTitleEl) appTitleEl.innerText = `سامانه تمرینی ${name} | Chieftain Pro`;
    }
    updateAdminUI();
  } else {
...
function restoreBackedUpProfile(userKey, skipPrompt = false) {
  if (userKey === 'hossein') {
    if (!skipPrompt) {
      const pwd = prompt(currentLang === 'en' ? 'Enter private access code:' : 'لطفاً کد دسترسی اختصاصی را وارد کنید:');
      if (pwd === null) return;
      if (pwd.trim() !== 'gym') {
        alert(currentLang === 'en' ? '❌ Invalid access code.' : '❌ کد دسترسی نادرست است.');
        return;
      }
    }
    ...
    localStorage.setItem('chieftain_admin_unlocked', 'true');
    activeProfileId = 'template_male';
    localStorage.setItem('chieftain_active_profile_id', 'template_male');

    // 1. Ensure Profile in allProfiles
    let hProf = allProfiles.find(p => p.id === 'template_male');
    if (!hProf) {
      hProf = JSON.parse(JSON.stringify(HOSSEIN_PROFILE));
      hProf.id = 'template_male';
      allProfiles.unshift(hProf);
    }
    saveProfiles();

    // 2. If Supabase is connected, sync everything from user's private cloud
    ...
    closeAuthModal();
    renderApp(true);
    showToast(currentLang === 'en' ? '🎉 Welcome! Routine loaded.' : '🎉 برنامه و تنظیمات با موفقیت بارگذاری شد!');
    ...
    activeProfileId = 'template_female';
    localStorage.setItem('chieftain_active_profile_id', 'template_female');

    // 1. Ensure Profile in allProfiles
    let mProf = allProfiles.find(p => p.id === 'template_female');
    if (!mProf) {
      mProf = JSON.parse(JSON.stringify(MORVARID_PROFILE));
      mProf.id = 'template_female';
      allProfiles.push(mProf);
    }
    saveProfiles();

    // 2. If Supabase is connected, sync
    if (typeof supabaseClient !== 'undefined' && supabaseClient && currentAuthUser) {
      syncCurrentDataWithSupabase();
    }
    ...
    closeAuthModal();
    renderApp(true);
    showToast(currentLang === 'en' ? '🎉 Welcome! Routine loaded.' : '🎉 برنامه با موفقیت بارگذاری شد!');
  }
}
