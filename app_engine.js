  ...
  return {
    ...
    chest: {
      ...
      evalStatus: (val) => {
        ...
        if (val >= H * 0.50) return { label: '🟢 محدوده سلامت', cls: 'lab-status-healthy' };
    ...
    armFlexed: {
      ...
      evalStatus: (val) => {
        ...
        if (val >= H * 0.18) return { label: '🟢 نرمال سلامت', cls: 'lab-status-healthy' };
    ...
    armRelaxed: {
      ...
      evalStatus: (val) => {
        ...
        if (val >= H * 0.16) return { label: '🟢 نرمال سلامت', cls: 'lab-status-healthy' };
        return { label: '⚠️ نیاز به هایپرتروفی', cls: 'lab-status-attention' };
      }
    },
    waist: {
    ...
    thigh: {
      ...
      evalStatus: (val) => {
        ...
        if (val >= H * 0.28) return { label: '🟢 نرمال سلامت', cls: 'lab-status-healthy' };
  ...
  if (list.length === 0) {
      ...
      <div class="metrics-hero-grid">
        ...
        <div class="metrics-hero-card">
          <div class="metric-hero-label">
            ...
            <span style="font-size:10px; color:#38bdf8;">نقطه شروع: ${baseline.weight}kg</span>
          </div>
          <div class="metric-hero-val">${current.weight} <small>kg</small></div>
          <div style="display:flex; justify-content:space-between; align-items:center; margin-top:4px;">
            <div style="font-size:10.5px; color:var(--text-muted);">تغییر از شروع:</div>
        ...
        <div class="metrics-hero-card">
          <div class="metric-hero-label">
            ...
            <span style="font-size:10px; color:#34d399;">ارتش آمریکا</span>
          </div>
        ...
        <div class="metrics-hero-card">
          ...
          <div style="display:flex; justify-content:space-between; align-items:center; margin-top:4px;">
            <div style="font-size:10.5px; color:var(--text-muted);">وضعیت:</div>
    ...
    const labReportBoxHtml = `
      <div class="lab-report-box">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; border-bottom:1px solid rgba(56,189,248,0.25); padding-bottom:8px; flex-wrap:wrap; gap:8px;">
          <div style="display:flex; align-items:center; gap:8px;">
            ...
            <div>
              ...
              <div style="font-size:11px; color:#94a3b8; margin-top:2px;">مقایسه با استانداردهای سلامت جهانی (WHO/Navy) و نسبت‌های طلایی فیزیک کلاسیک (قد: ${current.height || '180'}cm)</div>
            </div>
          </div>
          ...
          <table class="lab-table">
            <thead>
              <tr>
                ...
                <th style="text-align:center; color:#34d399;">محدوده نرمال سلامت 🟢</th>
                <th style="text-align:center; color:#facc15;">محدوده طلایی و زیبایی 👑</th>
        ...
        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(240px, 1fr)); gap:10px;">
          <div style="background:rgba(0,0,0,0.3); border:1px solid rgba(255,255,255,0.06); border-radius:10px; padding:10px;">
            <div style="font-size:11.5px; font-weight:800; color:#34d399; margin-bottom:4px;">🔬 ترکیب بدنی و چربی‌سوزی:</div>
            <div style="font-size:11px; color:#cbd5e1; line-height:1.6;">
              توده چربی: <b>${curSci.fatMass !== null ? curSci.fatMass + ' kg' : '--'}</b> · عضله خالص: <b>${curSci.leanMass !== null ? curSci.leanMass + ' kg' : '--'}</b>
              <div style="color:#38bdf8; margin-top:3px;">
                ${curSci.bodyFat !== null ? (curSci.bodyFat <= 14 ? 'تفکیک عضلانی فوق‌العاده و درصد چربی در محدوده طلایی فیتنس.' : 'روند چربی‌سوزی در حال پیشرفت است؛ تمرکز بر حفظ عضله خالص.') : 'درصد چربی پس از ثبت دور گردن و کمر محاسبه می‌شود.'}
              </div>
            </div>
          </div>
