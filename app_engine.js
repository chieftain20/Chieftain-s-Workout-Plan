...
function renderBodyMetricsView() {
  ...
  if (list.length === 0) {
      ...
      <div style="background:rgba(15,23,42,0.6); border:1px solid rgba(244,114,182,0.25); border-radius:10px; padding:12px;">
        ...
        <div style="display:flex; justify-content:space-around; align-items:center; background:rgba(0,0,0,0.25); border-radius:8px; padding:8px 4px; margin-bottom:8px;">
          ...
          <div style="text-align:center;">
            ...
            <div style="font-size:14px; font-weight:900; color:#34d399;">${curSci.whr || '--'}</div>
          </div>
        </div>
        <div style="font-size:11px; color:#cbd5e1; line-height:1.5;">
          🌸 <b>معیار زیبایی فیتنس بانوان:</b> بر خلاف بدنسازی مردانه، بازوی خانم‌ها هرگز نباید اندازه ساق پا (۳۷cm) ضخیم شود! بازوی <b>${current.armRight || current.armLeft || 31} سانتی‌متری</b> شما با دور کمر باریک و باسن در نهایت ظرافت زنانه قرار دارد. هدف: لیفت عضلات سرینی (Glutes) و بازوهای کشیده و توند بدون حجم زمخت.
        </div>
      </div>
    ` : `
    ...
    const strategyHtml = isFemale ? `
      ...
      <div style="background:rgba(15,23,42,0.8); border:1px solid rgba(244,114,182,0.25); border-radius:10px; padding:10px 14px; font-size:11px; color:#cbd5e1; line-height:1.6;">
        ...
        <div style="margin-top:4px; display:grid; grid-template-columns:repeat(auto-fit, minmax(200px, 1fr)); gap:8px;">
          ...
          <div>
            <span style="color:#34d399;">۳. تعادل هورمونی و درصد چربی ۱۸-۲۱٪:</span>
            حفظ ذخایر چربی ضروری استروژنیک برای سلامت چرخه قائدگی، پوست و مو.
          </div>
        </div>
      </div>
    ` : `
    ...
    const loveHandleBoxHtml = `
      <div style="background:linear-gradient(145deg, #0e172a, #162035); border:1px solid ${isFemale ? 'rgba(244,114,182,0.35)' : (isLoveHandleProminent ? 'rgba(244,63,94,0.35)' : 'rgba(56,189,248,0.35)')}; border-radius:var(--radius-lg); padding:16px; margin-bottom:16px; box-shadow:0 4px 20px rgba(0,0,0,0.35);">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; border-bottom:1px solid rgba(255,255,255,0.08); padding-bottom:8px; flex-wrap:wrap; gap:8px;">
          <div style="display:flex; align-items:center; gap:8px;">
            <span style="font-size:22px;">${isFemale ? '🌸' : '🎯'}</span>
            <div>
              <h3 style="font-size:14.5px; font-weight:900; color:${isFemale ? '#f472b6' : (isLoveHandleProminent ? '#fb7185' : '#38bdf8')}; margin:0;">
          ...
          <span style="font-size:11px; background:${isFemale ? 'rgba(244,114,182,0.15)' : (isLoveHandleProminent ? 'rgba(244,63,94,0.15)' : 'rgba(56,189,248,0.15)')}; color:${isFemale ? '#f472b6' : (isLoveHandleProminent ? '#f43f5e' : '#38bdf8')}; border:1px solid ${isFemale ? 'rgba(244,114,182,0.3)' : (isLoveHandleProminent ? 'rgba(244,63,94,0.3)' : 'rgba(56,189,248,0.3)')}; padding:3px 10px; border-radius:8px; font-weight:800;">
            ${isFemale ? (isLoveHandleProminent ? '⚠️ چربی ملایم پهلو' : '👑 ساعت‌شنی ظریف') : (isLoveHandleProminent ? '⚠️ لاو هندل شناسایی شد' : '🟢 توزیع نرمال V-Taper')}
          </span>
        </div>
