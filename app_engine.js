...
function isAdminUnlocked() {
  // 1. Server-verified role from Supabase profiles table or JWT app_metadata
  if (typeof serverAdminVerified !== 'undefined' && serverAdminVerified) {
    return true;
  }
  if (typeof currentAuthUser !== 'undefined' && currentAuthUser) {
  ...
  // 2. Local offline PIN unlock (Device-level UI gate for offline PWA)
...
function getActiveProfile() {
  return allProfiles.find(p => p.id === activeProfileId) || TEMPLATE_MALE_PROFILE;
}
  ...
  if (activeMainTab === 'workout' && workoutContent) {
    if (!workoutContent.innerHTML || workoutContent.innerHTML.trim() === '') {
      ...
      if (prof.id === 'morvarid' || prof.id === 'template_female') {
        prof.days = JSON.parse(JSON.stringify(TEMPLATE_FEMALE_PROFILE.days));
      } else {
        prof.days = JSON.parse(JSON.stringify(TEMPLATE_MALE_PROFILE.days));
      }
      saveProfiles();
...
function renderWorkoutDays() {
  ...
  if (!prof || !Array.isArray(prof.days) || prof.days.length === 0) {
    ...
    if (activeProfileId === 'morvarid' || activeProfileId === 'template_female') {
      prof = JSON.parse(JSON.stringify(TEMPLATE_FEMALE_PROFILE));
    } else {
      prof = JSON.parse(JSON.stringify(TEMPLATE_MALE_PROFILE));
    }
  ...
  if (!daysHtml || daysHtml.trim() === '') {
    const fallbackDays = (activeProfileId === 'morvarid' || activeProfileId === 'template_female' ? TEMPLATE_FEMALE_PROFILE.days : TEMPLATE_MALE_PROFILE.days);
...
function saveNewProfile() {
  ...
  let newDays = [];
  if (template === 'clone' || template === 'male') {
    newDays = JSON.parse(JSON.stringify(TEMPLATE_MALE_PROFILE.days));
  } else if (template === 'morvarid' || template === 'female') {
    newDays = JSON.parse(JSON.stringify(TEMPLATE_FEMALE_PROFILE.days));
  } else {
    newDays = [
...
function updateAuthUI(user) {
  ...
  if (user) {
    ...
    const name = currentUserProfile?.display_name || user.user_metadata?.display_name || user.email?.split('@')[0] || 'کاربر';
    if (btnText) {
      btnText.innerText = serverAdminVerified ? `👑 ${name}` : name;
    }
    if (btn) {
      btn.style.borderColor = serverAdminVerified ? '#fbbf24aa' : '#10b981aa';
      btn.style.color = serverAdminVerified ? '#fbbf24' : '#34d399';
      btn.style.background = serverAdminVerified ? 'rgba(245,158,11,0.18)' : 'rgba(16,185,129,0.18)';
    }
    ...
    if (nameDisplay) {
      const isEn = currentLang === 'en';
      const roleBadge = serverAdminVerified 
        ? (isEn ? ' (👑 Verified Admin)' : ' (👑 ادمین تایید شده سرور)') 
        : (isEn ? ' (Member)' : ' (عضو)');
      nameDisplay.innerText = isEn ? `Welcome, ${name}${roleBadge}` : `خوش آمدید، ${name}${roleBadge}`;
    }
...
async function fetchUserProfileAndRole(user) {
  ...
  try {
    ...
    console.error('Error querying profiles table in Supabase:', e);
    // Safe fallback to app_metadata if table query fails
